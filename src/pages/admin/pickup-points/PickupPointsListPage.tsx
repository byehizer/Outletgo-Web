import { Loader2, MapPin, Pencil, Plus, Power, PowerOff, Search } from 'lucide-react';
...
import { DeactivatePickupPointModal } from '../../../features/admin/DeactivatePickupPointModal';
import { PickupPointFormModal } from '../../../features/admin/PickupPointFormModal';
import {
  fetchPickupPoints,
  togglePickupPointStatus,
} from '../../../features/admin/pickupPointsApi';
import { useDebouncedValue } from '../../../hooks/useDebouncedValue';
import { useToast } from '../../../hooks/useToast';
import { ADMIN_PICKUP_POINTS_PAGE_SIZE } from '../../../lib/constants';
import { cn } from '../../../lib/cn';
import { ApiError } from '../../../lib/http/apiClient';
import type { Page } from '../../../types/api';
import type { PickupPoint } from '../../../types/pickup-point';

type PickupPointsListUiState = {
  data: Page<PickupPoint> | null;
  loading: boolean;
  errorMessage: string | null;
};

type PickupPointsListAction =
  | { type: 'FETCH_BEGIN' }
  | { type: 'FETCH_OK'; payload: Page<PickupPoint> }
  | { type: 'FETCH_ERR'; payload: string };

type PickupPointsPageModal =
  | { kind: 'create' }
  | { kind: 'edit'; pickupPoint: PickupPoint }
  | { kind: 'deactivate'; pickupPoint: PickupPoint }
  | null;

function pickupPointsListReducer(
  state: PickupPointsListUiState,
  action: PickupPointsListAction,
): PickupPointsListUiState {
  switch (action.type) {
    case 'FETCH_BEGIN':
...
const initialListState: PickupPointsListUiState = {
  data: null,
  loading: true,
  errorMessage: null,
};
...
function PickupPointsTableSkeleton() {
  return (
      ...
      <table className="min-w-[36rem] w-full border-collapse text-sm">
        <thead>
          <tr className="border-b border-[var(--border)] bg-[var(--bg-surface)]">
            {['Nombre', 'Dirección', 'Localidad/Barrio', 'Coordenadas', 'Horarios', 'Estado', 'Acciones'].map((col) => (
              <th
                key={col}
                scope="col"
...
export function PickupPointsListPage() {
  const { success, error: showError } = useToast();
  const [searchParams, setSearchParams] = useSearchParams();
  const [listState, dispatch] = useReducer(pickupPointsListReducer, initialListState);
  const [refreshNonce, setRefreshNonce] = useState(0);
  const [modal, setModal] = useState<PickupPointsPageModal>(null);
  const [reactivateTarget, setReactivateTarget] = useState<PickupPoint | null>(null);
  ...
  useEffect(() => {
    ...
    void fetchPickupPoints({
      ...
      size: ADMIN_PICKUP_POINTS_PAGE_SIZE,
      ...
      .catch((error: unknown) => {
        ...
        if (error instanceof ApiError) {
          ...
          dispatch({ type: 'FETCH_ERR', payload: 'No se pudo cargar el listado de puntos de retiro.' });
        }
      });
  ...
  const columns: DataColumn<PickupPoint>[] = useMemo(
    (): DataColumn<PickupPoint>[] => [
      {
        id: 'name',
        header: 'Nombre',
        wrap: true,
      ...
      },
      {
        id: 'address',
        header: 'Dirección',
        cell: (row) => <span className="text-[var(--text-secondary)]">{row.address}</span>,
      },
      {
        id: 'location',
      ...
      },
      {
        ...
        cell: (row) => <span className="text-[var(--text-secondary)]">{row.businessHours}</span>,
      },
      {
        id: 'status',
        header: 'Estado',
      ...
      },
      {
        ...
        cell: (row) => (
          <div className="flex items-center justify-end gap-1">
            ...
            <button
              type="button"
              aria-label={row.isActive ? 'Desactivar punto' : 'Reactivar punto'}
              className={cn(
              ...
              onClick={() => {
                if (row.isActive) {
                  setModal({ kind: 'deactivate', pickupPoint: row });
                } else {
  ...
  const handleReactivateConfirm = useCallback(async () => {
    ...
    try {
      await togglePickupPointStatus(reactivateTarget.id, true, { reason: '' });
      setReactivateTarget(null);
      success('Punto de retiro reactivado');
      bumpList();
    } catch (err: unknown) {
      if (err instanceof ApiError) {
        ...
        showError('No se pudo reactivar el punto de retiro.');
      }
    } finally {
    ...
    <div className="space-y-8">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-[var(--text-muted)]">
            Administrá los locales físicos y depósitos centrales de OutletGo donde se consolidan las compras.
          </p>
        </div>
        <button
          type="button"
      ...
      <section className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-6">
        ...
        <div className="mt-6">
          ...
          {showSkeleton ? <PickupPointsTableSkeleton /> : null}

          {!loading && data && data.content.length === 0 ? (
            <EmptyState
              title={
                hasSearch
                  ? 'No se encontraron puntos de retiro con esos criterios'
                  : 'No hay puntos de retiro registrados'
              }
            />
          ) : null}
      ...
      {modal?.kind === 'create' ? (
        <PickupPointFormModal
          ...
          onSuccess={() => handleModalSuccess('Punto de retiro creado correctamente')}
        />
      ) : null}
      ...
      {modal?.kind === 'deactivate' ? (
        <DeactivatePickupPointModal
          open
          pickupPoint={modal.pickupPoint}
          onClose={() => setModal(null)}
          onSuccess={() => handleModalSuccess('Punto de retiro desactivado')}
        />
      ) : null}

      <ConfirmDialog
        open={reactivateTarget != null}
        title="Reactivar punto de retiro"
        description={
          reactivateTarget ?
            <>
              ¿Reactivar el punto de retiro{' '}
              <span className="font-medium text-[var(--text-primary)]">{reactivateTarget.name}</span>
              ? Los compradores podrán volver a seleccionarlo para retirar sus pedidos.
            </>
          : null
        }
