  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col selection:bg-indigo-500 selection:text-white">
      ...
      <main className="flex-1 mx-auto max-w-4xl px-6 py-10 md:py-16 space-y-10 w-full">
        ...
        <div className="flex items-center justify-between gap-4 print:hidden">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2 text-xs font-bold text-white hover:bg-slate-800 shadow-sm transition active:scale-95"
        ...
        {/* ARTÍCULO PRINCIPAL (#printable-area para CSS @media print) */}
        <article id="printable-area" className="bg-white rounded-3xl border border-slate-200/80 p-6 md:p-12 shadow-sm space-y-8">
          {/* TÍTULO Y METADATOS */}
          <div className="space-y-4 border-b border-slate-100 pb-6">
            <h1 className="font-display text-2xl md:text-4xl font-extrabold text-slate-900 leading-tight">
              {article.title}
            </h1>
            <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500">
              ...
              <span>·</span>
              <span className="text-indigo-600 font-bold">{article.category}</span>
            </div>
          </div>

          {/* PORTADA DE IMAGEN */}
          <div className="aspect-[16/9] w-full rounded-2xl overflow-hidden bg-slate-100 border border-slate-100 shadow-sm">
            <img
              src={article.image}
              alt={article.title}
          ...
          {/* BOTÓN FINAL DE DESCARGA PDF */}
          <div className="pt-8 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 print:hidden">
            <div className="flex items-center gap-2 text-xs text-slate-500">
              <FileText className="size-4 text-indigo-500" />
              <span>¿Querés guardar este artículo offline?</span>
            </div>
            <button
              onClick={handleDownloadPDF}
              className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-slate-900 px-6 text-xs font-bold text-white hover:bg-slate-800 transition"
            >
              <Download className="size-4 text-emerald-400" />
              <span>Descargar nota en PDF</span>
            </button>
          </div>
        </article>
