...
import { togglePickupPointStatus } from './pickupPointsApi';
...
export type DeactivatePickupPointModalProps = {
  open: boolean;
...
export function DeactivatePickupPointModal({
  open,
  pickupPoint,
  onSuccess,
  onClose,
}: DeactivatePickupPointModalProps) {
  const titleId = useId();
  ...
  const onSubmit = async (values: DeactivateFormValues) => {
    ...
    try {
      await togglePickupPointStatus(pickupPoint.id, false, { reason: values.reason.trim() });
      onSuccess();
    } catch (error: unknown) {
      if (error instanceof ApiError) {
        ...
        setSubmitError('No se pudo desactivar el punto de retiro.');
      }
    }
  };
    ...
    >
      ...
      >
        <h2 id={titleId} className="font-display text-lg font-semibold text-[var(--text-primary)]">
          Desactivar punto de retiro
        </h2>

        <p className="mt-3 text-sm leading-relaxed text-[var(--text-secondary)]">
          Vas a desactivar el punto de retiro{' '}
          <span className="font-medium text-[var(--text-primary)]">{pickupPoint.name}</span> (
          {pickupPoint.address}, {pickupPoint.neighborhood}). Los compradores ya no podrán seleccionarlo para retirar compras.
        </p>

        <form className="mt-5 space-y-4" onSubmit={handleSubmit(onSubmit)} noValidate>
          <div className="space-y-2">
            <label htmlFor="deactivate-pp-reason" className="text-xs font-medium text-[var(--text-secondary)]">
              Motivo de desactivación
            </label>
            <textarea
              id="deactivate-pp-reason"
              rows={4}
              placeholder="Ej: Mantenimiento del local, reubicación física..."
              className={cn(
